...
def otherinteractions(l_bit, l_res_choices, st_protein, st_lig, l_serial_atom_backbone):
										...
										if atom.IsHbondDonor() & atomlig.IsHbondAcceptor():
											for neighborDon in OBAtomAtomIter(atom):
												if neighborDon.IsHydrogen():
													for neighborAcc in OBAtomAtomIter(atomlig):
														angle = neighborAcc.GetAngle(atomlig, neighborDon)
														if angle>135.0:
...
#														l_bit[residuename] |= bitarray('0001000')
										if atom.IsHbondAcceptor() & atomlig.IsHbondDonor():
											for neighborDon in OBAtomAtomIter(atomlig):
												if neighborDon.IsHydrogen():
													for neighborAcc in OBAtomAtomIter(atom):
														angle = neighborAcc.GetAngle(atom, neighborDon)
														if angle>135.0:
