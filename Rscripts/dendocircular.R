...
dendogramCircleClassAff = function (ddesc, daff, pfilout){
  ...
  dtypeplot =  as.data.frame(daff[,4])
  rownames(dtypeplot) = rownames(daff)
  ...
  open_tree(t2, 15) %>% rotate_tree(15)
  ggsave(paste(pfilout, "classAff.png", sep = ""), dpi=300, height = 11, width = 11)
...
dendogramCircle = function(ddes, daff, pfilout){
  ...
  t4 <- ggtree(tupgma2, layout="circular", size=2.2)
  t4 <- t4 %<+% daff + geom_text(aes(color=Aff, label=label, angle=angle, fontface="bold"), hjust=-0.05, size=4) +
    geom_tippoint(aes(color=Aff), alpha=0.75, size=1.5)+
    scale_color_continuous(low='red', high='lightgreen') +
    theme(legend.position="right")+
    theme(plot.margin = unit(c(0,0,0,0), "cm")) +
    geom_treescale(x = 1, y = 1, width = 1, offset = NULL,
                 color = "white", linesize = 1E-100, fontsize = 1E-100)
  print(t4)
  ggsave(pfilout, dpi=300, height = 14, width = 15)
